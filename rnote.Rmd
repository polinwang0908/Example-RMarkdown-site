...
- **Law of Large Number**: For any small $\epsilon>0$, $Pr(|\frac{L_1+L_2+...+L_N}{N}-\mu<\epsilon|)\rightarrow1$ as $N\rightarrow \infty$, where $L_i$ can be loss of person $i$ and all $L_i$ has mean $\mu$. This means when sample size is big enough, sample mean will approach the true mean.

LLN is useful when thinking about the more people are insured, the actual average loss is close to the true loss.

- **Central Limit Theorm**: Regardless of how $L_i$ distributes, $\frac{(\frac{L_1+L_2+...+L_N}{N}-\mu)}{\sigma/\sqrt{N}}\rightarrow$ Normal (0,1) as $N\rightarrow \infty$. This means the random variable, sample mean, will be very close to normal if sample size is big enough. 
